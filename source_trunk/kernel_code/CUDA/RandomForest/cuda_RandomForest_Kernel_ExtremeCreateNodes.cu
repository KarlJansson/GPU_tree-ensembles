...
namespace ExtremeCreateNodes{
	...
	__global__ void kernel_entry(paramPack_Kernel params){
		...
		unsigned int distStart = params.childIds[currentNode];
		int bestAttribute = params.attributes[currentNode];
		...
		unsigned int numNodeIndices = params.nodeIndicesLimits[(cb_constants.cb_nodeBufferEnd+1)+currentNode]-nodeIndStart+1;
		
		unsigned int numAttValues = params.attributeValCounts[bestAttribute];

		for(unsigned int i=0; i<((numAttValues >= max_nominal) ? 2 : numAttValues); ++i){
			nodeCounts[i] = params.distBuffer[distStart+(2*i)]+params.distBuffer[distStart+(2*i)+1];
		}
		...
		for(unsigned int i=0; i<((numAttValues >= max_nominal) ? 2 : numAttValues); ++i){
			dist[0] = params.distBuffer[distStart+(2*i)];
			dist[1] = params.distBuffer[distStart+(2*i)+1];
		...
		for(unsigned int i=0; i<((numAttValues >= max_nominal) ? 2 : numAttValues); ++i){
			...
			if(leafs[i]){
				// Create leaf node
				params.attributes[newNodes+numNewNodes] = -1;
				...
				// Create inner node
				params.attributes[newNodes+numNewNodes] = -2;
				params.treeIds[newNodes+numNewNodes] = sourceTree;
